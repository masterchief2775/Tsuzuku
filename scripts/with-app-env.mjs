...
function main(argv) {
  ...
  // On Windows, npm-installed binaries (vite, tsc, …) are `.cmd` shims.
  // `spawn` resolves a bare command name like "vite" through PATH only when
  // given a shell — without `shell: true` it fails with ENOENT even though
  // node_modules/.bin/vite.cmd exists. Harmless no-op on macOS/Linux, where
  // spawn already resolves bare executable names on PATH without a shell.
